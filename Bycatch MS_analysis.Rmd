...
CPUE.grsp%>%
  group_by(Species)%>%
  nest()%>%
  mutate(mod = map(data, ~glmer(cpue ~ Gear.Type + (1|Boat.Name), data = ., family = poisson())),
...
CPUE.grsp%>%
  group_by(Gear.Type)%>%
  nest()%>%
  mutate(mod = map(data, ~glmer(cpue ~ Species + (1|Boat.Name), data = ., family = poisson())),
...
CPUE.month%>%
  ...
  group_by(Gear.Type, Species)%>%
  nest()%>%
  mutate(mod = map(data, ~glmer(CPUE ~ Month + (1|Boat.Name), data = ., family = poisson())),
         sumry = map(mod, broom::tidy),
...
CPUE%>%
  group_by(Gear.Type, Species)%>%
  nest()%>%
  mutate(mod = map(data, ~glmer(CPUE ~ fe + (1|Boat.Name), data = ., family = poisson())),
         sumry = map(mod, broom::tidy))%>%
  select(sumry)%>%
  unnest()
```
